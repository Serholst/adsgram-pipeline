...
- `agent-system/reference/outreach-templates.md` — Product Card (v3, DEFAULT) vs Personalized Pitch (v2).
  Choosing logic, body templates, formats/clients tables, P.S. rules, все примеры, CTA tables, signature.
- `agent-system/reference/outreach-rules.md` — pre-pitch research, forbidden elements, language rules,
  pre-send checklists (Product Card + Personalized Pitch), **output format**, CRM update rules.
