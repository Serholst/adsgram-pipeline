...
AdsGram is a Telegram-native advertising platform with **225M+ views** across **1,000+ resources** (channels, bots, and mini apps) in **60+ countries**. The value proposition for advertisers: access to Telegram's 950M+ user base through native ad formats (rewarded ads in mini-apps, sponsored messages) with wallet-based targeting. The target customer is someone who buys digital traffic — media buyers, UA managers, growth leads — at companies in verticals where Telegram audiences convert well: iGaming, VPN, Forex, Crypto, Adult.
...
- **AdsGram value**: Telegram as a new paid channel on top of their current mix — wallet-based targeting, platform CPM from $0.9–$3.0 (varies by GEO), fast launch
- **If the lead is clearly buying traffic** (Media Buyer, UA Manager) → direct pitch with GEO-specific metrics
- **If the lead is in growth/marketing but unclear if they buy traffic** → qualification-first approach
...
- **Asia:** India, Indonesia, Philippines, Vietnam, Bangladesh, Pakistan
- **Europe:** Italy, Spain, CIS region (Russia, Ukraine, Belarus, Turkey)
- **LATAM:** Brazil, Mexico, Argentina, Colombia
- **Middle East:** Iran, Iraq, Saudi Arabia, Yemen

### Traffic Benchmarks by GEO (for lead prioritization)

**Total platform shows: 225,796,954 (~20 months of data)**
**Formula:** `225.8M / 20 × country_traffic_% = monthly views`

| Region | Country | Shows (%) | CTR (%) | CPM ($) | Monthly Views |
|--------|---------|-----------|---------|---------|---------------|
| Europe | Russia | 20.6 | 1.2 | 2.5 | 2.33M |
| Africa | Nigeria | 13.3 | 2.5 | 1.2 | 1.50M |
| Asia | India | 10.0 | 1.7 | 1.2 | 1.13M |
| Europe | Ukraine | 9.6 | 1.3 | 1.5 | 1.08M |
| Asia | Vietnam | 6.5 | 0.8 | 1.0 | 734K |
| Asia | Indonesia | 6.2 | 1.3 | 1.7 | 700K |
| Asia | Bangladesh | 6.2 | 2.1 | 1.4 | 700K |
| Asia | Pakistan | 3.6 | 2.0 | 0.9 | 406K |
| Europe | Turkey | 2.7 | 1.4 | 2.1 | 305K |
| Asia | Philippines | 2.4 | 1.1 | 1.3 | 271K |
| Africa | Ethiopia | 1.8 | 2.9 | 1.6 | 203K |
| Europe | Belarus | 1.8 | 1.3 | 1.9 | 203K |
| Africa | Algeria | 1.8 | 2.2 | 1.1 | 203K |
| Asia | Uzbekistan | 1.8 | 1.9 | 2.9 | 203K |
| Middle East | Iran | 1.5 | 2.4 | 1.4 | 169K |
| Africa | Egypt | 1.5 | 1.8 | 1.3 | 169K |
| Europe | Poland | 0.9 | 1.5 | 1.3 | 102K |
| Europe | Germany | 0.9 | 1.9 | 2.1 | 102K |
| Asia | Myanmar | 0.8 | 1.3 | 1.3 | 90K |
| Asia | Kazakhstan | 0.7 | 1.3 | 3.0 | 79K |
| Middle East | Iraq | 0.7 | 2.3 | 1.2 | 79K |
| S. America | Brazil | 0.7 | 1.4 | 2.5 | 79K |
| Africa | Ghana | 0.7 | 1.8 | 1.0 | 79K |
| Asia | Japan | 0.6 | 0.7 | 1.1 | 68K |
| Asia | South Korea | 0.6 | 1.7 | 2.1 | 68K |
| Africa | Morocco | 0.6 | 2.5 | 0.9 | 68K |
| Europe | Armenia | 0.5 | 1.6 | 2.2 | 56K |
| Middle East | Yemen | 0.5 | 2.0 | 0.9 | 56K |
| Middle East | Saudi Arabia | 0.5 | 1.8 | 2.0 | 56K |

**Use this table to prioritize prospecting efforts:** Countries with higher traffic share have more inventory to sell → leads there are higher value. GEOs with <100K monthly views may not support large campaigns — flag this when presenting leads.

**Case Study (for context when evaluating leads):**
...
**Step 1b: Load the CRM contact set.** If you haven't already loaded the CRM in this session, do it now. The CRM is at `AdsGram_CRM.xlsx` in the mounted folder. Extract all existing contacts into a dedup set:
...
print(f"CRM dedup set loaded: {len(crm_emails)} emails, {len(crm_names)} name+company pairs")
```

**Keep `crm_emails` and `crm_names` in memory for the entire session.** These sets are used at two checkpoints:

1. **Before enrichment (Stage 4):** Filter out any leads whose email or name+company already exist in CRM. This prevents wasting credits on re-enrichment.
2. **Before writing to CRM (Stage 5):** Final dedup check to prevent duplicate rows.
...
**Step 1d: Validation gate — run before ANY Apollo people search.** This is a hard requirement. After assembling your candidate domain list, run this check programmatically:

```python
# candidate_domains = ["domain1.com", "domain2.com", ...]  ← your proposed search list
blocked = []
approved = []
for d in candidate_domains:
    if d.strip().lower() in exclusion_domains:
        blocked.append(d)
    else:
        approved.append(d)

if blocked:
    print(f"BLOCKED ({len(blocked)} domains already in Top_iGaming_Operators.xlsx):")
    for b in blocked:
        print(f"  ✗ {b}")
print(f"\nAPPROVED for search ({len(approved)} domains):")
for a in approved:
    print(f"  ✓ {a}")
```

Only domains that appear in the `approved` list may be sent to Apollo. If `blocked` is non-empty, inform the user which domains were filtered out and why.

This gate exists because in previous sessions the exclusion check was done visually and companies from the database were accidentally searched, wasting time and requiring manual cleanup.
