...
**The Pre-Enricher agent handles this stage.** See `agent-system/agents/pre-enricher/AGENT.md` for the full 10-step pipeline. Key outputs that feed into Stage 2:
...
**Output:** `agent-system/contracts/pre-enricher-output.json` — passed to Orchestrator, who extracts `search_vectors_for_apollo` and feeds them to Searcher alongside the standard search request.
...
```json
{
  "excluded_domains": [...],    // domains to SKIP (Prospected non-empty OR Search Results contains "excluded")
  "excluded_count": N,
  "available_domains": [...],   // domains in DB that are still available for search
  "available_count": N
}
```

Store `excluded_domains` as `exclusion_domains` — you'll need it throughout the session. The `available_domains` list shows companies in the DB that haven't been prospected yet and can be searched.
...
Store `excluded_domains` as `exclusion_domains` — you'll need it throughout the session. The `available_domains` list shows companies in the DB that haven't been prospected yet.
