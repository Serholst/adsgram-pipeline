...
Прочитай config/agent-config.md. Тебе нужно:
...
contracts/crm-writer-input.json из двух источников:

**Bucket A** (из qualifier-output.json, где `bucket: "A"`):
...
- `lead_status` ← маппинг verification_status (Enricher прокидывает
  его из qualifier-output — используй прокинутое значение)
