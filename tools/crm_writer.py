...
Reads /tmp/pipeline/crm-writer-input.json, validates, deduplicates,
sorts by priority, writes to Google Sheets CRM, updates Company DB.

Usage:
    python3 tools/crm_writer.py [--dry-run]

Input:  /tmp/pipeline/crm-writer-input.json
Output: stdout JSON + /tmp/pipeline/crm-writer-output.json
"""
...
INPUT_PATH = Path("/tmp/pipeline/crm-writer-input.json")
OUTPUT_PATH = Path("/tmp/pipeline/crm-writer-output.json")
...
def update_company_db(leads: list[dict], dry_run: bool) -> dict:
    ...
    tmp_file = "/tmp/pipeline/companies_batch.json"
...
def main():
    ...
    if crm_rows and not args.dry_run:
        tmp_file = "/tmp/pipeline/leads_batch.json"
        with open(tmp_file, "w") as f:
...
def _write_output(result: dict):
    ...
    Path("/tmp/pipeline").mkdir(parents=True, exist_ok=True)
